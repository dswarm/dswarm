...
public class EmbeddedServer {
	...
	public HttpServer start(boolean skipStart) throws IOException {
		...
		final HttpServer server = GrizzlyHttpServerFactory
				.createHttpServer(getBaseUri(), false);
		server.getListener("grizzly").setMaxFormPostSize(Integer.MAX_VALUE);


		final WebappContext context =
				new WebappContext("DMP 2000 Backend", getContextPath());
		...
		servletRegistration.setInitParameter("javax.ws.rs.Application",
				"de.avgl.dmp.controller.providers.DMPApplication");
	...
	public String getContextPath() {
		...
		if (value != null) {
			...
			log.warn("Value of " + CONTEXT_PATH_PROPERTY +
					" property must start with a '/' [" + value + "]." +
					" Using default [" + DEFAULT_CONTEXT_PATH + "].");
		}
	...
	public int getPort() {
		...
		if (value != null) {
			...
			try {
				...
				log.warn("Value of " + HTTP_PORT_PROPERTY +
						" property is not a  valid positive integer [" + value + "]." +
						" Using default [" + DEFAULT_PORT + "].", e);
			}
		}
