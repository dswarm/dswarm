...
public class AttributePathServiceTest extends IDBasicJPAServiceTest<AttributePath, AttributePathService, Long> {
	...
	public void testSimpleAttributePath() {
		...
		System.out.println("attribute title = '" + attributePath.toString());
		...
		Assert.assertEquals("the first attributes of the attribute path are not equal", attributePath.getAttributes().get(0), updatedAttributePath
				.getAttributes().get(0));
